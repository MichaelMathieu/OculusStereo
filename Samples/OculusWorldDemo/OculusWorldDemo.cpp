...
long i_iter = 0;
unsigned char* imgdatas[2] = {NULL, NULL};
int imgdatas_i = 0;
void OculusWorldDemoApp::RenderEyeView(ovrEyeType eye)
{
    ...
    */
    ...
    Ptr<Node> pnode = VideoScene1[0].World.Nodes[0];
    Fill* pfill = ((Model*)(Node*)pnode)->Fill;
    Texture* tex = pfill->GetTexture(0);
    Texture* tex2 = pRender->CreateTexture(Texture_RGBA, w, h, imgdata);
    pfill->SetTexture(0, tex2);
    GLuint texId = ((OVR::Render::GL::Texture*)tex)->TexId;
    //glBindTexture(GL_TEXTURE_2D, texId);
    //glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, imgdata);
    //Note node = VideoScene1[0].World.Nodes[0]->fill->SetTexture(tex);
    //VideoScene1[(i_iter/2000)%3].Render(pRender, view);
    VideoScene1[0].Render(pRender, view);
