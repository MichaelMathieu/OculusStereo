...
void OculusWorldDemoApp::PopulateVideo1() {
  ...
  filenames.push_back(fileName + "_blueCube.tga");
  filenames.push_back(fileName + "_redCube.tga");
  filenames.push_back(fileName + "_LoadScreen.tga");

  for (int i = 0; i < filenames.size(); ++i) {
    Ptr<File> imageFile = *new SysFile(filenames[i]);
    if (!imageFile->IsValid()) {
      printf(String("Invalid file ") + filenames[i] + " \n");
      exit(0);
    }
    Ptr<Texture> imageTex;
    imageTex = *LoadTextureTga(pRender, imageFile);
    ...
    VideoScene1.push_back(Scene());
    VideoScene1.back().World.Add(m);
  }
